...
class GeneralSettingController extends AdminBaseController
{
    ...
    public function intro_screen(){
        return view('admin.generalsetting.intro-screen');
    }

    public function splash_screen(){
        return view('admin.generalsetting.splash_screen');
    }
    ...
    public function appVersion(){
        return view('admin.generalsetting.app-version');
    }
    ...
    public function generalupdate(Request $request)
    {
        ...
        $data = Generalsetting::findOrFail(1);
            ...
            if ($file = $request->file('splash_screen'))
            {
                $name = \PriceHelper::ImageCreateName($file);
                $data->upload($name,$file,$data->splash_screen);
                $input['splash_screen'] = $name;
            }
            if ($file = $request->file('intro_screen_1'))
            {
                $name = \PriceHelper::ImageCreateName($file);
                $data->upload($name,$file,$data->intro_screen_1);
                $input['intro_screen_1'] = $name;
            }
            if ($file = $request->file('intro_screen_2'))
            {
                $name = \PriceHelper::ImageCreateName($file);
                $data->upload($name,$file,$data->intro_screen_2);
                $input['intro_screen_2'] = $name;
            }
            if ($file = $request->file('intro_screen_3'))
            {
                $name = \PriceHelper::ImageCreateName($file);
                $data->upload($name,$file,$data->intro_screen_3);
                $input['intro_screen_3'] = $name;
            }
