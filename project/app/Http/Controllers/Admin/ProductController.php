...
class ProductController extends AdminBaseController
{
    ...
    {
        $cats = Category::all();
        $dukans = Dukan::where('status', 1)->get();
        $sign = $this->curr;
        if ($slug == 'physical') {
            return view('admin.product.create.physical', compact('cats', 'dukans', 'sign'));
        } else if ($slug == 'digital') {
            return view('admin.product.create.digital', compact('cats', 'dukans', 'sign'));
        } else if (($slug == 'license')) {
            return view('admin.product.create.license', compact('cats', 'dukans', 'sign'));
        }
    }
    ...
    public function store(Request $request)
    {
        ...
        // Check Packing
        if (in_array(null, $request->packing_size) || in_array(null, $request->packing_price) || in_array(null, $request->packing_measure)) {
            $input['packing_size'] = null;
            $input['packing_price'] = null;
            $input['packing_measure'] = null;
        } else {
            $input['packing_size'] = implode(',', str_replace(',', ' ', $request->packing_size));
            $input['packing_price'] = implode(',', str_replace(',', ' ', $request->packing_price));
            $input['packing_measure'] = implode(',', str_replace(',', ' ', $request->packing_measure));
        }

        // Check Packing
        if (empty($request->dukan_id)) {
            $input['dukan_id'] = '';
        } else {
            $input['dukan_id'] = implode(',', $request->dukan_id);
        }
    ...
    public function edit($id)
    {
        ...
        if ($data->type == 'Digital') {
            return view('admin.product.edit.digital', compact('cats', 'data', 'dukans', 'sign'));
        } elseif ($data->type == 'License') {
            return view('admin.product.edit.license', compact('cats', 'data', 'dukans', 'sign'));
        } else {
            if(!is_null($data->packing_measure)){
                $packingmeasure = explode(',', $data->packing_measure);
            }
            if(!is_null($data->packing_size)){
                $packingsize = explode(',', $data->packing_size);
            }
            if(!is_null($data->packing_price)){
                $packingprice = explode(',', $data->packing_price);
            }
            if(!is_null($data->dukan_id)){
                $dukanArr = explode(',', $data->dukan_id);
            }

            return view('admin.product.edit.physical', compact('cats', 'data', 'dukans', 'sign', 'packingmeasure', 'packingsize', 'packingprice', 'dukanArr'));
        }
    ...
    public function update(Request $request, $id)
    {
        ...
        if (!in_array(null, $request->packing_size) || !in_array(null, $request->packing_price) || !in_array(null, $request->packing_measure)) {
            ...
            if (in_array(null, $request->packing_size) || in_array(null, $request->packing_price) || in_array(null, $request->packing_measure)){
                ...
                $input['packing_measure'] = implode(',', $request->packing_measure);           
                $input['packing_size'] = implode(',', $request->packing_size);           
                $input['packing_price'] = implode(',', $request->packing_price);           
            }
        }
